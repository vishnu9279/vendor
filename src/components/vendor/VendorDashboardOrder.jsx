...
const VendorDashboardOrder = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // console.log("getVendorOrder response" , response);

      const resposeParsing = JSON.parse(response.data.data)
      console.log("getVendorOrder data parsinf" , resposeParsing);
      setUserOrder(resposeParsing.orders);
      ...
      <div className="mx-auto mt-8 max-w-2xl md:mt-12">
        <div className="bg-white shadow-lg">
          ...
          <div className="flex flex-col justify-between ml-4 flex-grow">
            ...
            <span className="text-red-500 text-sm">{item?.dialCode}{item?.phoneNumber}</span>
          ...
          <div className="flex flex-row  ml-4 flex-grow mt-2">
            <img
              ...
              className="w-5 h-4"
            />
            <span className="font-bold text-sm"> {item?.addressInfo.address}</span>
          </div>
          <div className="px-4 py-6 sm:px-8 sm:py-10">
            <div className="flow-root">
              <span className="h-1 w-full bg-slate-400 lg:w-1/3"></span>
              <ul className="-my-8">
                <span className="mt-10 font-bold text-slate-400 text-sm">Order ID:- #{item?.orderId}</span>
                <div>
